...
import assignment2_ProductManagementSystem.Product;
import assignment2_ProductManagementSystem.ProductFileHandler;
	...
	private void attachEventListeners() {
		...
		buttonsMap.get("Add").addActionListener(evt -> {
			...
			try {
				...
				try {
					productId = Integer.parseInt(textFieldsMap.get("Product ID").getText());
				} catch (NumberFormatException ex) {
					throw new IllegalArgumentException("Product ID must be a valid integer");
				}


				name = textFieldsMap.get("Name").getText();
				...
				try {
					quantity = Integer.parseInt(textFieldsMap.get("Quantity in hand").getText());
				} catch (NumberFormatException ex) {
					throw new IllegalArgumentException("Quantity in hand must be a valid integer");
				}


				try {
					price = Double.parseDouble(textFieldsMap.get("Unit Price").getText());
				} catch (NumberFormatException ex) {
					throw new IllegalArgumentException("Unit Price must be a valid number");
				}
